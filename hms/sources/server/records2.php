...
//查询信息
    $sql = "SELECT COUNT(orderid) AS `lines` FROM checkin_info ".
    ...
    mysqli_free_result($query);
        ...
        $a=($page-1)*16;
        $sql = "SELECT DISTINCT checkin_info.*, guests.sex, FLOOR(DATEDIFF(now(),guests.birthday)/365.25) as age, guests.telephone ".
            "FROM checkin_info, guests ".
            "WHERE checkin_info.guestid=guests.guestid ".
        ...
        while ($row=mysqli_fetch_assoc($query)) {
            $recordsData.=sprintf($records,$row['orderid'],$row['name'],$row['sex']=='F'?'女':'男',$row['age'],$row['telephone'],$row['roomid'],substr($row['time'],0,10),$row['predictdays'],$row['bill']);
        } 
        mysqli_free_result($query);
