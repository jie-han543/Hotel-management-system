...
//查询信息
    $sql = "SELECT COUNT(orderid) AS `lines` FROM order_info ".
    ...
    mysqli_free_result($query);
        ...
        $a=($page-1)*16;
        $sql = "SELECT DISTINCT order_info.*, ".
            " if(isnull(order_info.groupid),guests.sex,'') AS sex, ".
            " if(isnull(order_info.groupid),FLOOR(DATEDIFF(now(),guests.birthday)/365.25),'') AS age, ".
            " if(isnull(order_info.groupid),guests.telephone,groups.contacttel) AS tel ".
            "FROM order_info, guests, groups ".
            "WHERE (order_info.guestid=guests.guestid OR order_info.groupid=groups.groupid) ".
        ...
        while ($row=mysqli_fetch_assoc($query)) {
            $recordsData.=sprintf($records,$row['orderid'],$row['name'],$row['sex']=='F'?'女':'男',$row['age'],$row['tel'],$row['roomid'],substr($row['time'],0,10),$row['predictdays']);
        } 
        mysqli_free_result($query);
